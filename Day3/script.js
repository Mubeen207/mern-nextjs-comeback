...
function updateSummary() {
  ...
  document
    .querySelector(".balance-card")
    .classList.toggle("negative", balance < 0);
}
...
function renderTransactions() {
  ...
  if (!visible.length) {
    transactionList.innerHTML = `<div class="empty-state"><strong>${transactions.length ? "No matching transactions" : "Your ledger is clear"}</strong>${transactions.length ? "Try a different search or filter." : "Add your first entry to see it here."}</div>`;
    return;
  }
...
function resetForm() {
  ...
  submitButton.innerHTML = "Add transaction <span>↗</span>";
  warning.textContent = "";
}
...
form.addEventListener("submit", (event) => {
  ...
  if (!title || !amount || amount < 0) {
    warning.textContent = "Please enter a description and a valid amount.";
    return;
  }
  const data = {
    title,
...
transactionList.addEventListener("click", (event) => {
  ...
  if (button.dataset.action === "delete") {
    ...
    submitButton.innerHTML = "Update transaction <span>↗</span>";
    titleInput.focus();
  }
  saveTransactions();
  updateSummary();
...
[searchInput, typeFilter, categoryFilter].forEach((control) =>
